...
class GeneLCSApp(tk,Tk): #define a janela principal da aplicação
    ...
    def init_main_screen(self):
        ...
        import_button = tk.Button(frame, text="Importar FASTA", font=self.font_button,
                                bg="#FF69B4", fg="white",
    ...
    def show_color_map(self): #mostra cada caracter das sequências alinhadas com cores
        ...
        for i in range(alignment_len):
            column_chars = [s[i] for s in seqs]
            if all(c == column_chars[0] for c in column_chars) and column_chars[0] != "-":
                color = "green"
            elif "-" in column_chars:
                color = "yellow"
            else:
                color = "red"
            colors.append(color)
        ...
        for row, seq in enumerate(seqs):
            for col, char in enumerate(seq):
                lbl = tk.Label(color_map_frame, text=char, bg=colors[col], fg="black", font=("Courier New", 10), width=2)
...
if _name_ == "_main_": #inicializa a aplicação carregando o ficheiro 
